...
public:
    /*
            void quad(double a,double b1,double c,double *sr,double *si, double
       *lr,double *li) void fxshfr(int l2, int *nz) void quadit(double
       *uu,double *vv,int *nz) void realit(double *sss, int *nz, int *iflag)
    ...
    */
    ...
    _110:
        ...
        for (jj = 0; jj < 5; jj++)
        {
            ...
            if (!zerok)
            {
                /*  Use a scaled form of recurrence if value of k at 0 is
                 * nonzero. */
                t = -aa / cc;
                for (i = 0; i < nm1; i++)
                {
