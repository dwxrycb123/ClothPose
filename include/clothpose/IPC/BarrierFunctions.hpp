#ifndef BarrierFunctions_hpp
#define BarrierFunctions_hpp
//
...
#include <cmath>
#include <iostream>
#define BARRIER_FUNC_TYPE 2
namespace IPC
{
inline void b_C0(double d, double dHat, double& b) { b = -log(d / dHat); }
...
inline void H_bC2(double d1, double dHat1, double& H)
{
    ...
    /*     19-Jun-2019 15:02:54 */
    t2 = d1 - dHat1;
    H = (std::log(d1 / dHat1) * -2.0 - t2 * 4.0 / d1) +
        1.0 / (d1 * d1) * (t2 * t2);
}
...
inline void compute_b(double d, double dHat, double& b)
{
    ...
    {
        std::cout << "needs to define BARRIER_FUNC_TYPE to be 0 (C0 clamped "
                     "log) or 2 (C2 clamped log)"
                  << std::endl;
    }
}
