...
public:
    virtual void set_pattern(const std::vector<std::set<int>>& vNeighbor)
    {
        ...
        tbb::parallel_for(
            0, (int)vNeighbor.size(), 1,
            [&](int vI)
            {
                ja_v[vI].resize((vNeighbor[vI].size() + 1) * 3);
                ...
                ja_v[vI].conservativeResize(nnz * 3 - 3);
        ...
        else
        {
            for (int rowI = 0; rowI < numRows; ++rowI)
            {
                for (const auto& colIter : other->IJ2aI[rowI])
                {
                    ...
                    {
                        a[finder->second] +=
                            multiplier * other->a[colIter.second];
                    }
                }
            }
        }
