...
public:
    template <int dim>
    static void addBlockToMatrix(
        const Eigen::Matrix<double, dim, dim *(dim + 1)> &block,
        ...
        LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *linSysSolver)
    {
        ...
        if (index[1] >= 0)
        {
            ...
            linSysSolver->addCoeff(rowStart + 1, _dimIndex1 + 1,
                                   block(1, dim + 1));
            linSysSolver->addCoeff(rowStart, _dimIndex1 + 2, block(0, dim + 2));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex1 + 2,
                                   block(1, dim + 2));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex1, block(2, dim));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex1 + 1,
                                   block(2, dim + 1));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex1 + 2,
                                   block(2, dim + 2));
        }
        ...
        if (index[2] >= 0)
        {
            ...
            linSysSolver->addCoeff(rowStart, _dimIndex2 + 1,
                                   block(0, _2dim + 1));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex2, block(1, _2dim));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex2 + 1,
                                   block(1, _2dim + 1));
            linSysSolver->addCoeff(rowStart, _dimIndex2 + 2,
                                   block(0, _2dim + 2));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex2 + 2,
                                   block(1, _2dim + 2));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex2, block(2, _2dim));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex2 + 1,
                                   block(2, _2dim + 1));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex2 + 2,
                                   block(2, _2dim + 2));
        }
        ...
        if (index[3] >= 0)
        {
            ...
            linSysSolver->addCoeff(rowStart, _dimIndex3 + 1,
                                   block(0, _3dim + 1));
            linSysSolver->addCoeff(rowStart, _dimIndex3 + 2,
                                   block(0, _3dim + 2));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex3, block(1, _3dim));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex3 + 1,
                                   block(1, _3dim + 1));
            linSysSolver->addCoeff(rowStart + 1, _dimIndex3 + 2,
                                   block(1, _3dim + 2));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex3, block(2, _3dim));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex3 + 1,
                                   block(2, _3dim + 1));
            linSysSolver->addCoeff(rowStart + 2, _dimIndex3 + 2,
                                   block(2, _3dim + 2));
        }
    }
    ...
    {
        Eigen::JacobiSVD<Eigen::Matrix<Scalar, size, size>> svd(
            mtr, Eigen::ComputeFullU | Eigen::ComputeFullV);
        ...
        mtr = U * Eigen::DiagonalMatrix<Scalar, size>(svd.singularValues()) *
              V.transpose();
    }
    ...
    static void writeSparseMatrixToFile(const std::string &filePath,
                                        ...
                                        bool MATLAB = false);
    static void writeSparseMatrixToFile(
        const std::string &filePath,
        const std::map<std::pair<int, int>, double> &mtr, bool MATLAB = false);
    static void writeSparseMatrixToFile(
        const std::string &filePath,
        LinSysSolver<Eigen::VectorXi, Eigen::VectorXd> *linSysSolver,
        bool MATLAB = false);
    ...
    static bool readSEG(const std::string &filePath, Eigen::MatrixXd &V,
                        Eigen::MatrixXi &E);
    static void writeSEG(const std::string &filePath, const Eigen::MatrixXd &V,
                         const Eigen::MatrixXi &E);

    static bool segTriIntersect(const Eigen::RowVector3d &ve0,
                                const Eigen::RowVector3d &ve1,
                                const Eigen::RowVector3d &vt0,
                                const Eigen::RowVector3d &vt1,
                                const Eigen::RowVector3d &vt2)
    {
        ...
        {
            return false;  // edge is on one side of the plane that triangle is
                           // in
        }
...
#endif
        Eigen::Vector3d uvt =
            coefMtr.fullPivLu().solve((ve0 - vt0).transpose());
    ...
    static bool pointBehindTri(const Eigen::RowVector3d &vt0,
                               const Eigen::RowVector3d &vt1,
                               const Eigen::RowVector3d &vt2,
    ...
    static bool pointInsideTetrahedron(const Eigen::RowVector3d &v0,
                                       const Eigen::RowVector3d &vt0,
                                       const Eigen::RowVector3d &vt1,
                                       const Eigen::RowVector3d &vt2,
                                       const Eigen::RowVector3d &vt3)
    {
#ifdef USE_PREDICATES
        igl::predicates::exactinit();
        ...
        if (ori1 != igl::predicates::Orientation::NEGATIVE)
        {
            ...
            if (ori2 != igl::predicates::Orientation::NEGATIVE)
            {
                ...
                {
                    const auto ori4 =
                        igl::predicates::orient3d(vt1, vt2, vt3, v0);
    ...
    static void buildSTri2Tet(const Eigen::MatrixXi &F,
    ...
    static void saveTetMesh_vtk(const std::string &filePath,
                                const Eigen::MatrixXd &TV,
                                const Eigen::MatrixXi &TT);
    static bool readTetMesh(const std::string &filePath, Eigen::MatrixXd &TV,
                            Eigen::MatrixXi &TT, Eigen::MatrixXi &F,
                            bool findSurface = true);
    ...
    static void readNodeEle(const std::string &filePath, Eigen::MatrixXd &TV,
                            Eigen::MatrixXi &TT, Eigen::MatrixXi &F);

    template <int colSize>
    static void dF_div_dx_mult(
        const Eigen::Matrix<double, DIM * DIM, colSize> &right,
        const Eigen::Matrix<double, DIM, DIM> &A,
        Eigen::Matrix<double, DIM *(DIM + 1), colSize> &result, bool symmetric)
    {
        ...
        if (symmetric)
        {
            ...
            result(2, 1) = result(1, 2) =
                right(0, 1) * A(0, 0) + right(1, 1) * A(0, 1);
            result(3, 1) = result(1, 3) = _2100 + _3101;
            result(4, 1) = result(1, 4) =
                right(0, 1) * A(1, 0) + right(1, 1) * A(1, 1);
        ...
        for (int colI = 0; colI < right.cols(); colI++)
        {
            ...
            result(5, colI) =
                (A.row(0) * right.block(DIM * 2, colI, DIM, 1))[0];
            result(6, colI) = (A.row(1) * right.block(0, colI, DIM, 1))[0];
            result(7, colI) = (A.row(1) * right.block(DIM, colI, DIM, 1))[0];
            result(8, colI) =
                (A.row(1) * right.block(DIM * 2, colI, DIM, 1))[0];
            result(9, colI) = (A.row(2) * right.block(0, colI, DIM, 1))[0];
            result(10, colI) = (A.row(2) * right.block(DIM, colI, DIM, 1))[0];
            result(11, colI) =
                (A.row(2) * right.block(DIM * 2, colI, DIM, 1))[0];
            result(0, colI) =
                -result(3, colI) - result(6, colI) - result(9, colI);
            result(1, colI) =
                -result(4, colI) - result(7, colI) - result(10, colI);
            result(2, colI) =
                -result(5, colI) - result(8, colI) - result(11, colI);
        }
#endif
    }
    static void dF_div_dx_mult(
        const Eigen::Matrix<double, DIM, DIM> &right,
        const Eigen::Matrix<double, DIM, DIM> &A,
    ...
    static void Init_Dirichlet(const Eigen::MatrixXd &X,
