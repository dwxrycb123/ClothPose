...
#include <clothpose/IPC/CTCD/rpoly.h>
    ...
    if (roots > 0)
    {
        ...
        for (int i = 0; i < roots - 1; i++)
        {
            if (!((time[i] < 0 && time[i + 1] < 0) ||
                ...
                checkInterval(time[i], time[i + 1], op, reducedDegree,
                              intervals, pos);
        }
        if (time[roots - 1] <= 1.0)
            checkInterval(time[roots - 1], 1.0, op, reducedDegree, intervals,
                          pos);
    }
    else
    {
                        ...
                        const Eigen::Vector3d &p1end, double eta, double &t)
{
    ...
    // time intervals during which v is colinear with the edge, on the side of
    // e1 towards e2, and on the side of e2 towards e1
            ...
            for (int k = 0; k < (int)a1.size(); k++)
            {
                for (int l = 0; l < (int)b0.size(); l++)
                {
                    for (int m = 0; m < (int)b1.size(); m++)
                    {
                        ...
                        {
                            TimeInterval isect =
                                TimeInterval::intersect(intervals);
                            bool skip = false;
        ...
        for (int j = 0; j < (int)e1.size(); j++)
        {
            for (int k = 0; k < (int)e2.size(); k++)
            {
                for (int l = 0; l < (int)e3.size(); l++)
                {
                    ...
                    if (TimeInterval::overlap(intervals))
                    {
                        mint = std::min(TimeInterval::intersect(intervals).l,
                                        mint);
                        col = true;
                    }
                }
