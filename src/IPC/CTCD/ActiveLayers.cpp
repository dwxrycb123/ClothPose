#include <clothpose/IPC/CTCD/ActiveLayers.h>
#include <clothpose/IPC/CTCD/CTCD.h>
#include <clothpose/IPC/CTCD/CTCDNarrowPhase.h>
#include <clothpose/IPC/CTCD/Distance.h>
#include <clothpose/IPC/CTCD/History.h>
#include <clothpose/IPC/CTCD/KDOPBroadPhase.h>
#include <clothpose/IPC/CTCD/Mesh.h>
#include <clothpose/IPC/CTCD/PenaltyGroup.h>
#include <clothpose/IPC/CTCD/SeparatingPlaneNarrowPhase.h>
#include <clothpose/IPC/CTCD/SimulationState.h>
#include <clothpose/IPC/CTCD/TrivialBroadPhase.h>
...
bool PenaltyGroupComparator::operator()(const PenaltyGroup *first,
                                        const PenaltyGroup *second) const
{
    return second->nextFireTime() < first->nextFireTime();
}

ActiveLayers::ActiveLayers(double outerEta, double innerEta, double baseDt,
                           double baseStiffness, double terminationTime,
                           double CoR, bool verbose)
    : outerEta_(outerEta),
      innerEta_(innerEta),
      baseDt_(baseDt),
      baseStiffness_(baseStiffness),
      termTime_(terminationTime),
      CoR_(CoR),
      deepestLayer_(0),
      history_(NULL),
      verbose_(verbose)
...
{
    for (std::vector<PenaltyGroup *>::iterator it = groups_.begin();
         it != groups_.end(); ++it)
        delete *it;
...
void ActiveLayers::addGroups(int maxdepth)
{
    while (deepestLayer_ < maxdepth)
    {
        ...
        PenaltyGroup *newgroup =
            new PenaltyGroup(dti, etai, innerEta_, ki, CoR_);
...
bool ActiveLayers::step(SimulationState &s)
{
    ...
    if (!groupQueue_.empty())
    {
        ...
        {
            groupQueue_.pop();
            for (set<int>::iterator it = group->getGroupStencil().begin();
                 it != group->getGroupStencil().end(); ++it)
            {
                int vert = *it;
                for (int j = 0; j < 3; j++)
                {
                    ...
                    newq[3 * vert + j] =
                        s.q[3 * vert + j] +
                        (newtime - s.lastUpdateTime[3 * vert + j]) *
                            s.v[3 * vert + j];
                    newv[3 * vert + j] =
                        (newq[3 * vert + j] - s.q[3 * vert + j]) / dt;
                }
            }
                 ...
                 it != group->getGroupStencil().end(); ++it)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    if (F[3 * vert + j] != 0.0) touched = true;
                }
                if (touched)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        s.v[3 * vert + j] +=
                            s.minv[3 * vert + j] * F[3 * vert + j];
                        s.q[3 * vert + j] = newq[3 * vert + j];
                        s.lastUpdateTime[3 * vert + j] = newtime;
                    }

                    history_->addHistory(vert, newtime,
                                         s.q.segment<3>(3 * vert));
                }
            }
...
void ActiveLayers::rollback()
{
    ...
    std::priority_queue<PenaltyGroup *, std::vector<PenaltyGroup *>,
                        PenaltyGroupComparator>
        newqueue;
...
double ActiveLayers::VFStencilThickness(VertexFaceStencil stencil)
{
    ...
    if (it != vfdepth_.end()) return layerDepth(it->second + 1);
    return layerDepth(1);
}
...
double ActiveLayers::EEStencilThickness(EdgeEdgeStencil stencil)
{
    ...
    if (it != eedepth_.end()) return layerDepth(it->second + 1);
    return layerDepth(1);
}

bool ActiveLayers::collisionDetection(const Mesh &m,
                                      set<VertexFaceStencil> &vfsToAdd,
                                      set<EdgeEdgeStencil> &eesToAdd,
                                      const set<int> &fixedVerts)
{
    vfsToAdd.clear();
    eesToAdd.clear();
    ...
    bp_->findCollisionCandidates(*history_, m, outerEta_, vfsToAdd, eesToAdd,
                                 fixedVerts);
    if (verbose_)
        std::cout << "Broad phase found " << vfsToAdd.size()
                  << " vertex-face and " << eesToAdd.size()
                  << " edge-edge candidates" << std::endl;
    set<pair<VertexFaceStencil, double> > etavfs;
    set<pair<EdgeEdgeStencil, double> > etaees;
    for (set<VertexFaceStencil>::iterator it = vfsToAdd.begin();
         it != vfsToAdd.end(); ++it)
    {
        pair<VertexFaceStencil, double> vp(*it, VFStencilThickness(*it));
        etavfs.insert(vp);
    }
    for (set<EdgeEdgeStencil>::iterator it = eesToAdd.begin();
         it != eesToAdd.end(); ++it)
    {
...
bool ActiveLayers::runOneIteration(const Mesh &m, SimulationState &s)
{
    if (verbose_)
    {
        std::cout << "Taking an outer iteration, deepest layer is currently "
                  << deepestLayer_;
        if (deepestLayer_)
            std::cout << " with outer thickness "
                      << groups_[deepestLayer_ - 1]->getOuterEta() << " and dt "
                      << groups_[deepestLayer_ - 1]->getDt();
        std::cout << std::endl;
    }
    delete history_;
    ...
    if (verbose_)
        std::cout << "Done simulating, accumulated "
                  << history_->countHistoryEntries() << " history entries"
                  << std::endl;
    ...
    {
        if (s.minv[i] == 0.0) fixedVerts.insert(i / 3);
    }

    bool collisions = collisionDetection(m, vfsToAdd, eesToAdd, fixedVerts);
    if (verbose_)
        std::cout << "Found " << vfsToAdd.size() << " vertex-face and "
                  << eesToAdd.size() << " edge-edge collisions" << std::endl;

    rollback();

    for (std::set<VertexFaceStencil>::iterator it = vfsToAdd.begin();
         it != vfsToAdd.end(); ++it)
        addVFStencil(*it);
    for (std::set<EdgeEdgeStencil>::iterator it = eesToAdd.begin();
         it != eesToAdd.end(); ++it)
        addEEStencil(*it);
