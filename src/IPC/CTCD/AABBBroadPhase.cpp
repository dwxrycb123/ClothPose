#include <clothpose/IPC/CTCD/AABBBroadPhase.h>
...
void AABBBroadPhase::findCollisionCandidates(const History &h, const Mesh &m,
                                             double outerEta,
...
AABBNode *AABBBroadPhase::buildAABBTree(const History &h, const Mesh &m,
                                        double outerEta)
{
    ...
    for (int i = 0; i < (int)m.faces.cols(); i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
                 ...
                 it != h.getVertexHistory(verts[j]).end(); ++it)
            {
                ...
                {
                    node->mincorner[k] =
                        min(it->pos[k] - outerEta, node->mincorner[k]);
                    node->maxcorner[k] =
                        max(it->pos[k] + outerEta, node->maxcorner[k]);
                }
            }
        }
...
AABBNode *AABBBroadPhase::buildAABBInterior(vector<AABBNode *> &children)
{
    ...
    for (vector<AABBNode *>::iterator it = children.begin();
         it != children.end(); ++it)
    {
        for (int j = 0; j < 3; j++)
        {
...
void AABBBroadPhase::intersect(AABBNode *left, AABBNode *right, const Mesh &m,
