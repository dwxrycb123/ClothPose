...
void KDOPBroadPhase::findCollisionCandidates(const History &h, const Mesh &m,
                                             double outerEta,
...
KDOPNode *KDOPBroadPhase::buildKDOPTree(const History &h, const Mesh &m,
                                        double outerEta)
{
    ...
    for (int i = 0; i < (int)m.faces.cols(); i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            for (vector<HistoryEntry>::const_iterator it =
                     h.getVertexHistory(verts[j]).begin();
                 it != h.getVertexHistory(verts[j]).end(); ++it)
            {
                for (int k = 0; k < K; k++)
                {
                    node->mins[k] =
                        min(it->pos.dot(DOPaxis[k]) - outerEta, node->mins[k]);
                    node->maxs[k] =
                        max(it->pos.dot(DOPaxis[k]) + outerEta, node->maxs[k]);
                }
            }
        }
...
KDOPNode *KDOPBroadPhase::buildKDOPInterior(vector<KDOPNode *> &children)
{
    ...
    assert(nchildren > 0);
    if (nchildren == 1) return children[0];
    ...
    for (vector<KDOPNode *>::iterator it = children.begin();
         it != children.end(); ++it)
    {
        for (int j = 0; j < K; j++)
        {
    ...
    int child = 0;
    for (; child < nchildren / 2; child++) left.push_back(children[child]);
    node->left = buildKDOPInterior(left);
    vector<KDOPNode *> right;
    for (; child < nchildren; child++) right.push_back(children[child]);
    node->right = buildKDOPInterior(right);
    return node;
}

void KDOPBroadPhase::intersect(KDOPNode *left, KDOPNode *right, const Mesh &m,
                               ...
                               const std::set<int> &fixedVerts)
{
    ...
    else
    {
        ...
        if (m.neighboringFaces(lleft->face, lright->face)) return;
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            bool allrfixed = true;
            alllfixed =
                alllfixed && fixedVerts.count(m.faces.coeff(i, lleft->face));
            allrfixed =
                allrfixed && fixedVerts.count(m.faces.coeff(i, lright->face));
            for (int j = 0; j < 3; j++)
            {
                alllfixed = alllfixed &&
                            fixedVerts.count(m.faces.coeff(j, lright->face));
                allrfixed = allrfixed &&
                            fixedVerts.count(m.faces.coeff(j, lleft->face));
            }
            if (!alllfixed)
                vfs.insert(VertexFaceStencil(m.faces.coeff(i, lleft->face),
                                             m.faces.coeff(0, lright->face),
                                             m.faces.coeff(1, lright->face),
                                             m.faces.coeff(2, lright->face)));
            if (!allrfixed)
                vfs.insert(VertexFaceStencil(m.faces.coeff(i, lright->face),
                                             m.faces.coeff(0, lleft->face),
                                             m.faces.coeff(1, lleft->face),
                                             m.faces.coeff(2, lleft->face)));
            for (int j = 0; j < 3; j++)
            {
                bool allefixed = true;
                allefixed = allefixed &&
                            fixedVerts.count(m.faces.coeff(i, lleft->face));
                allefixed = allefixed && fixedVerts.count(m.faces.coeff(
                                             (i + 1) % 3, lleft->face));
                allefixed = allefixed &&
                            fixedVerts.count(m.faces.coeff(j, lright->face));
                allefixed = allefixed && fixedVerts.count(m.faces.coeff(
