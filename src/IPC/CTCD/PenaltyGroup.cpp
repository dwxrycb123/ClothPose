...
PenaltyGroup::PenaltyGroup(double dt, double outerEta, double innerEta,
...
bool PenaltyGroup::addForce(const Eigen::VectorXd &q, const Eigen::VectorXd &v,
                            Eigen::VectorXd &F)
{
    VectorXd groupforce(q.size());
    groupforce.setZero();
    bool newused = false;
    for (vector<VertexFaceStencil>::iterator it = vfstencils_.begin();
         it != vfstencils_.end(); ++it)
    {
        bool fired = VertexFacePenaltyPotential::addForce(
            q, v, groupforce, *it, outerEta_, innerEta_, stiffness_, CoR_);
        newused |= (fired && it->isnew);
    }
    for (vector<EdgeEdgeStencil>::iterator it = eestencils_.begin();
         it != eestencils_.end(); ++it)
    {
        bool fired = EdgeEdgePenaltyPotential::addForce(
            q, v, groupforce, *it, outerEta_, innerEta_, stiffness_, CoR_);
        newused |= (fired && it->isnew);
    }
...
void PenaltyGroup::incrementTimeStep() { nextstep_++; }

double PenaltyGroup::nextFireTime() const { return nextstep_ * dt_; }
